...
const createPost = async (req, res) => {
  try {
    ...
    sendError(res, err.status || 500, err.message || "Failed to create post");
  }
};
...
const deletePost = async (req, res) => {
  try {
    const result = await postsService.deletePost(req.params.id, req.user._id);
    if (!result) return sendError(res, 404, "Post not found");
    sendResponse(res, 200, "Post deleted");
  } catch (err) {
    console.error("deletePost error:", err);
    sendError(res, err.status || 500, err.message || "Failed to delete post");
  }
};

const toggleLike = async (req, res) => {
  try {
    const result = await postsService.toggleLike(req.params.id, req.user._id);
    if (!result) return sendError(res, 404, "Post not found");
