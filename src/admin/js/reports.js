...
async function loadReports() {
    ...
    tbody.innerHTML =
        '<tr><td colspan="7" class="px-6 py-12 text-center text-gray-400"><i class="fas fa-spinner fa-spin mr-2"></i>Đang tải...</td></tr>';
    ...
    try {
        ...
        if (reports.length === 0) {
            tbody.innerHTML =
                '<tr><td colspan="7" class="px-6 py-12 text-center text-gray-400">Không có báo cáo nào</td></tr>';
        } else {
            tbody.innerHTML = reports
                .map((r) => {
                    ...
                    const targetContent =
                        r.targetType === "post"
                            ? escapeHtml(
                                  (r.targetPost?.content || "Đã xóa").substring(
                                      0,
                                      50,
                                  ),
                              )
                            : r.targetType === "comment"
                              ? escapeHtml(
                                    (
                                        r.targetComment?.content || "Đã xóa"
                                    ).substring(0, 50),
                                )
                    ...
                    const deleteCommentBtn =
                        r.targetType === "comment" && r.targetComment
                            ? `<button onclick="confirmAction('Xóa bình luận vi phạm', 'Xóa bình luận bị báo cáo và đánh dấu đã giải quyết?', function(){ return deleteReportedComment('${r._id}'); })"
                        class="btn-action px-3 py-1.5 text-xs font-medium bg-red-50 text-red-600 rounded-lg hover:bg-red-100" title="Xóa bình luận">
                        <i class="fas fa-trash"></i>
                    </button>`
                            : "";
...
async function deleteReportedComment(id) {
    try {
        await api("DELETE", `/admin/reports/${id}/delete-comment`);
        showToast("Đã xóa bình luận vi phạm", "bg-red-500");
        loadReports();
        loadDashboard();
    } catch (err) {
        showToast(err.message, "bg-red-500");
